...
vector<vector<vector<uint32_t>>> get_known_length_canvas(
    vector<vector<uint32_t>>& seq,
    vector<vector<bool>>& keep,
    vector<uint32_t> n,
    uint32_t blank_id) {
  vector<vector<uint32_t>> batch_canvas, batch_blanks, batch_rest, batch_loc, batch_lb;
  for (uint32_t b = 0; b < seq.size(); b++) {
    vector<uint32_t> canvas, blanks, rest, loc, lb;
    for (uint32_t i = 0; i < n[b]; ) {
      if (keep[b][i]) {
        canvas.push_back(seq[b][i]);
        i++;
        ...
        while (i < n[b] && !keep[b][i]) {
          rest.push_back(i);
          loc.push_back(blanks.size());
          lb.push_back(cur_len);
          i++;
          cur_len++;
        }
        blanks.push_back(canvas.size());
        canvas.push_back(blank_id + cur_len);
      }
    }
    batch_canvas.push_back(canvas);
    batch_blanks.push_back(blanks);
  ...
  return {batch_canvas, batch_blanks, batch_rest, batch_loc, batch_lb};
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("get_canvas", &get_canvas, "get_canvas");
  m.def("get_insertion_canvas", &get_insertion_canvas, "get_insertion_canvas");
  m.def("get_known_length_canvas", &get_known_length_canvas, "get_known_length_canvas");
